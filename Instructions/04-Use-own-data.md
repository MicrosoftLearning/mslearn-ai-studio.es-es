...
1. Repite los pasos anteriores para implementar un modelo **GPT-4** con el nombre de implementación `gpt-4` mediante una implementación **estándar** de la versión predeterminada con un límite de velocidad de TPM de 5K.
...
1. Revisa el código del archivo, teniendo en cuenta que:
    ...
    - Envía una indicación (incluido el sistema y un mensaje de usuario basado en la entrada de usuario) al cliente de Azure OpenAI y agrega información adicional sobre el índice de Búsqueda de Azure AI que se usará para fundamentar la indicación.
...
1. Cuando se te solicite, escribe una pregunta, como `Where should I stay in London?` y revisa la respuesta del modelo de IA generativa.
...
1. Prueba una pregunta de seguimiento, por ejemplo `What can I do there?`.
